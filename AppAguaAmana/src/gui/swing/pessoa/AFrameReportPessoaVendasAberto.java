...
public class AFrameReportPessoaVendasAberto  extends JFrame {
	...
	public AFrameReportPessoaVendasAberto(int id, String data_inicial, String data_final) {
		this.id = id;
		this.data_final = data_final;
		this.data_inicial = data_inicial;
		setTitle("Relatório de Cliente (Vendas)");
	...
	public void initComponents(){
		try {
			...
			print = JasperFillManager.fillReport("./relatorios-src/cliente_amana_vendas_aberto.jasper", parametros, FabricaConexao.getConexao());
            viewer = new JRViewer(print);
            add(viewer);
            pack();
		} catch (JRException e) {
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, "Não foi possível gerar relatório de cliente.", FabricaConexao.getApelido(), JOptionPane.ERROR_MESSAGE);
			dispose();
		}
	}
}
