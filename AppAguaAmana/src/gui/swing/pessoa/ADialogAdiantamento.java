...
public class ADialogAdiantamento extends JDialog {
	...
	public ADialogAdiantamento(int id){
		novoPagamento = false;
		setTitle("Adiantamento para o cliente " + Integer.toString(id));
	...
	public void initComponents(){
		setLayout(null);
		painelTopo = new APanelTopo("Pessoa", "Adiantamento", "imagens/32x32/pagamento.png");
		painelTopo.setBounds(0, 0, 320, 52);
		
		JPanel panel = new JPanel();
		panel.setLayout(null);
		panel.setBorder(BorderFactory.createTitledBorder("Dados do Adiantamento"));
				...
				try{
					...
					if(valor.signum() == 0){
						...
						if(FabricaDao.getPessoaDao().efetuarAdiantamento(adiantamento) != -1){
							novoPagamento = true;
                			JOptionPane.showMessageDialog(null, "Adiantamento efetuado com sucesso.", "Efetuar Adiantamento", JOptionPane.INFORMATION_MESSAGE);      
                       		setVisible(false);
                       		removeAll();
                       		dispose();
                		}
